namespace SamarPlanner.Identity.Application.Tests.UseCases.Commands.RegisterOrLogin;
...
public class RegisterOrLoginCommandHandlerTests
{
    ...
    public async Task Handle_NewUser_WithValidData_ShouldRegisterAndReturnToken()
    {
        ...
        _userRepository.GetAsync(ValidPhone, Arg.Any<CancellationToken>())
            .Returns((Core.Entities.User?)null);

        _userRepository.CreateAsync(Arg.Any<Core.Entities.User>(), ValidPassword, Arg.Any<CancellationToken>())
            .Returns(new IdentityResult(true, new Dictionary<string, string[]>()));
        
        _jwtTokenService.GenerateToken(Arg.Any<Core.Entities.User>())
    ...
    public async Task Handle_NewUser_ShouldCallCreateAsyncOnce()
    {
        ...
        await _userRepository.Received(1).CreateAsync(Arg.Any<Core.Entities.User>(), ValidPassword, Arg.Any<CancellationToken>());
    }
    ...
    public async Task Handle_ExistingUser_ShouldNotCallCreateAsync()
    {
        ...
        await _userRepository.DidNotReceive().CreateAsync(Arg.Any<Core.Entities.User>(), ValidPassword, Arg.Any<CancellationToken>());
    }
    ...
    public async Task Handle_NewUserWithWrongPassword_ShouldNotCallGenerateTokenAsync()
    {
        ...
        _jwtTokenService.DidNotReceive().GenerateToken(Arg.Any<Core.Entities.User>());
    }
